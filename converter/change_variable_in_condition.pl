...
################################################
# 
# change_variable_in_condition.pl
#
...
sub change_variable_in_condition{
    my ($ruby_hash, $target_variable, $control_variable) = @_;
	...
	foreach my $sentence_hash ($PARSE_RUBY->get_all_sentences($ruby_hash)){
	    ...
	    change_variable_in_condition($sentence_hash, $target_variable, $control_variable);
	}
	...
	change_variable_in_condition($loop, $max_var, $control_var);
	...
	if(defined($target_variable) && defined($control_variable)){
	    my ($h, $result) = 
		$PARSE_RUBY->replace_variable_into($if_clause, $target_variable, $control_variable);
	    $CHANGED = 1 if($result);
	}

	my $then_clause = $PARSE_RUBY->get_then_clause($ruby_hash);
	change_variable_in_condition($then_clause, $target_variable, $control_variable);

	#
	my $else_clause = $PARSE_RUBY->get_else_clause($ruby_hash);
	return if(!defined($else_clause));
	change_variable_in_condition($else_clause, $target_variable, $control_variable);
    }
}
